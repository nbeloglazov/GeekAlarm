...
public final class TaskManager {
	...
	private static Category jsonToCategory(JSONObject json) throws JSONException {
	...
	public static Task getTask(Category category, int level) throws Exception {
		String url = String.format("task?category=%s&level=%d", category.getCode(), level);
		...
		for (int i = 0; i < 4; i++){
			String choiceUrl = String.format("image?id=%s&type=choice&number=%d", id, i + 1);
